...
for(const d of devotions) {
 ...
 if(!d.videos?.length)throw Error('Missing videos: '+d.slug);
 const kinds=new Set();
...
function card(d,prefix='',heading='h3') {
 ...
 return `<a class="card" href="${prefix}${route(d)}" data-find="${esc(find)}"><span class="th"><img src="https://i.ytimg.com/vi/${d.videos[0].id}/hqdefault.jpg" alt="" loading="lazy" width="480" height="360"><span class="pl" aria-hidden="true"></span></span><span class="tx"><span class="passage">${esc(d.passage)}<span class="mins"> · ${d.readingMinutes} min read</span></span><${heading}>${esc(d.title)}</${heading}><span class="card-summary">${esc(d.summary)}</span><span class="card-action">Read devotion · ${d.videos.length>1?'Short & full film':'Short video'}</span></span></a>`;
}
...
${content}
<!-- DEVOTION CONTENT END -->
<section class="watch-section" id="watch"><h2>Watch this Bible story</h2><p>Reflect on the story in the accompanying ${d.videos.length>1?'Short and full film':'Short'} from I Can Still Believe.</p>${videos}</section>
...
const featured=`<section id="featured" aria-labelledby="latest-title"><div class="featured"><div class="fx"><span class="k">Latest devotion</span><h2 id="latest-title">${esc(latest.title)}</h2><div class="sub">${esc(latest.passage)} · ${latest.readingMinutes} min read</div><p>${esc(latest.summary)}</p><p class="verse">${esc(latest.keyVerse)}</p><p class="ref">${esc(latest.keyVerseReference)}</p><div class="btns"><a class="btn indigo" href="${route(latest)}">Read the devotion</a><a class="btn wine" href="${videoRoute(latest,latest.videos[0])}">Watch the Short</a></div></div><div class="fm"><a href="${videoRoute(latest,latest.videos[0])}" aria-label="Watch ${esc(latest.videos[0].title)}"><img src="https://i.ytimg.com/vi/${latest.videos[0].id}/hqdefault.jpg" alt="" width="480" height="360" fetchpriority="high"><span class="tag">Short</span><span class="play" aria-hidden="true"></span></a></div></div></section>`;
